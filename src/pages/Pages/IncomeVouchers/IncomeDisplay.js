...
import IncomeUnApprove from "../UnApproved/UnApproveIncome";
...
const IncomeDisplay = (props) => {
  ...
  useEffect(() => {
    ...
    async function fetchJournalDetails() {
      try {
        ...
        const response = await IncomeService.view(id);
  ...
  return (
    <React.Fragment>
      <LayoutWithBreadCrumb title=" Income Details" pageTitle="Income Voucher">
        <Container fluid>
          {/* <Button
            className="btn btn-soft-success mb-3 ml-auto"
            // Adjust the marginLeft and marginRight
            style={{ marginLeft: 0 }}
            onClick={() => navigate(`/income-vouchers`)}
          >
            Back to List
          </Button> */}
          <Row>
                ...
                <CardHeader>
                  <div className="row">
                    <div className="mb-5">
                      ...
                      <p className="h6 text-center">INCOME DETAILS</p>
                    </div>
                    <div className="gap-2">
                      <Col lg={12}>
                        <FormGroup row>
