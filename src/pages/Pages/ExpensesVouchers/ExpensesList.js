...
function ExpensesList(props) {
  ...
  async function fetchPosts() {
    try {
      const fetchedPosts = await ExpensesService.getList();
  ...
  const handleNavigation = () => {
    navigate("/expenses-vouchers/create");
  };

  const handleDelete = async (voucherNo) => {
    try {
      if (window.confirm("Do you really want to delete this ExpensesList?")) {
        await ExpensesService.delete(voucherNo);
  ...
  return (
    <React.Fragment>
      ...
      <Container fluid>
        <BreadCrumb title="Expense Vouchers" pageTitle="Expense"/>
          <Row>
            <Col lg={12}>
              <Card>
                <CardHeader>
                      ...
                      >
                        <thead className="table-light">
                          <tr>
                            ...
                            <th className="sort" data-sort="totalDebit">
                              Total Debit
                            </th>
                          ...
                          {MyDatalist.slice(
                            ...
                            <tr key={item.id}>
                              ...
                              <td className="voucherNo">
                                <Link
                                  to={`/expenses-vouchers/details/${item.voucherNo}`}
                                >
                                  {item.voucherNo}
                                </Link>
                              </td>
                              ...
                              <td className="text-end totalDebit">
                              ...
                              <td className="action">
                                <ButtonGroup>
                                  <div className="d-flex gap-1">
                                    ...
                                    <Link
                                      to={`/expenses-vouchers/edit/${item.voucherNo}`}
                                      state={{ item }}
                                    >
                                      <Button
