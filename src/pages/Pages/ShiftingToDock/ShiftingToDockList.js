...
function ShiftingToDockList(props) {
  const [shiftingToDockList, setShiftingToDockList] = useState([]);
  const [error, setError] = useState(null);
  const pageSize = 5; // Set the number of items per page
  const [currentPage, setCurrentPage] = useState(0);
  const navigate = useNavigate();

  async function fetchShiftingToDockList() {
    try {
      const fetchedList = await ShiftingToDockService.getList();
      setShiftingToDockList(fetchedList);
    } catch (err) {
      setError(err);
    }
  }
  ...
  const handleNavigationForm = () => {
    navigate("/shiftingtodock-form");
  };

  const handleDelete = async (id) => {
    try {
      if (window.confirm("Do you want to delete this Shifting to Dock List?")) {
        await ShiftingToDockService.delete(id);
        setShiftingToDockList((prevData) =>
  ...
  const handleNextClick = () => {
    const maxPage = Math.ceil(shiftingToDockList.length / pageSize);
  ...
  useEffect(() => {
    fetchShiftingToDockList();
  }, []);

  return (
    <div className="page-content">
      <BreadCrumb
        title="Shifting To Dock List"
        pageTitle="Shifting To Dock Records"
      />
      <Container fluid>
        <Row>
          <Col>
            <Card>
              ...
              <CardBody>
                            ...
                            <td className="action">
                              <ButtonGroup>
                                <div className="d-flex gap-1">
                                  <Link
                                    to={`/shiftingtodock-form/details/${item.id}`}
                                    state={{ item }}
                                  >
                                    <Button
                                  ...
                                  <Link
                                    to={`/shiftingtodock-form/edit/${item.id}`}
                                    state={{ item }}
                                  >
                                    <Button
                ...
                <TablePagination
                  pagesCount={Math.ceil(shiftingToDockList.length / pageSize)}
...
export default ShiftingToDockList;
