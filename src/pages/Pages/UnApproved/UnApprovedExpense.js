...
// import UnApprovedVoucherListService from "../AccountingService.js/UnApprovedVoucherList";

const UnApprovedExpenseList = (props) => {
  ...
  //   navigate("/expenses-vouchers/create"); // Replace with the actual path
  // };
  ...
  async function fetchPosts(id) {
    try {
      const response = await UnApprovedVoucherListService.ExpensegetList(id);
      console.log("data");
            ...
            <tbody>
              {MyDatalist.map((item, index) => (
                <tr key={item.id}>
                  ...
                  <td>
                    {" "}
                    <Link to={`/expenses-vouchers/details/${item.voucherNo}`}>
                      {item.voucherNo}
                    </Link>
                  </td>
...
export default UnApprovedExpenseList;
