...
import AvailableModal from "./Component/AvailableModal";
...
const Calender = () => {
  ...
  console.log("🚀 ~ Calender ~ driverData:", driverData);
  useEffect(() => {
  ...
  const driverOptions = [
    ...(driverData?.map((item) => ({
      value: item.id,
      label: `${item.name} ${item.surname}`,
      isAdmin: item.is_admin_driver, // Store is_admin flag in options
    })) || []),
  ];
  console.log("🚀 ~ Calender ~ driverOptions:", driverOptions);

  const adminDriver = driverOptions.find((driver) => driver.isAdmin);
  console.log("🚀 ~ Calender ~ adminDriver:", adminDriver);
  ...
  console.log("🚀 ~ Calender ~ selectedDriver:", selectedDriver);
  // Set default driver when driverData is loaded
  useEffect(() => {
    const adminDriver = driverOptions.find((driver) => driver.isAdmin);
    if (adminDriver) {
      setSelectedDriver(adminDriver);
    }
  }, [driverData]);
  ...
  const fetchEvent = async (month, year) => {
    try {
      const response = await EventService.getList(month, year);
  ...
  const handleDateClick = (arg) => {
    if (clickTimeout) {
      ...
      clickTimeout = setTimeout(() => {
        clickTimeout = null;
      }, 200); // Timeout to differentiate single and double clicks
    }
  };
  ...
  const handleEventClick = (arg) => {
    ...
    // console.log("🚀 ~ handleEventClick ~ payment_status:", payment_status);
  ...
  const toggleFilter = (event) => {
    // Prevent closing filter when interacting with dropdowns or react-select menu
    if (
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth >= 768) {
        setIsFilterOpen(true); // Keep filter open on large screens
      } else {
        setIsFilterOpen((prev) => prev); // Prevent unintended closing
      }
    };
  ...
  const handleDriverChange = (selectedOption) => {
    setSelectedDriver(
      selectedOption || driverOptions.find((driver) => driver.isAdmin)
    );

    // Ensure the filter remains open when selecting a driver
    setIsFilterOpen(true);
  };

  // on hover of event open the modal of event's detail====================
    ...
    return (
      <div style={containerStyle}>
        <div style={titleStyle}>
          {/* Display icon if payment_status is 1 */}
          ...
          {/* Display event title or full name based on view */}
        ...
        {!((isMobile && isDayGridView) || isTimeGridWeek) && (
          <img src={avatarPath} alt="Avatar" style={avatarStyle} />
        )}
      </div>
    );
  };
            ...
            <Col xs={12}>
                ...
                <Col lg={9} xs={12}>
                  ...
                  {(isFilterOpen || window.innerWidth >= 768) && (
                    <Row>
                      ...
                      <Col md={4}>
                        <p className="fs-5 text-center mb-0">Payment Status</p>
                      ...
                      <Col md={4} xs={12} className="mb-2">
                        <p className="fs-5 text-center mb-0">Instructor</p>
                        <div style={{ position: "relative" }}>
                          ...
                          <Select
                            ...
                            // isClearable={selectedDriver?.value !== }
              ...
              <Row>
                <Col xs={12} className="px-0 px-md-3 mt-2 mt-md-0 mx-0 mx-md-1">
                  {isMobile ? (
                    <div ref={calendarRef} className="bg-white py-4 mx-0">
                      <FullCalendar
                          ...
                          <FullCalendar
                            ...
                            views={{
                              ...
                              timeGridWeek: {
                                ...
                                slotDuration: "00:30:00", // 15-minute intervals
                                slotMinTime: "06:00:00",
                                slotMaxTime: "20:00:00",
                                allDaySlot: false, // Hide all-day section
                                slotLabelFormat: {
          ...
          {selectedEvent ? (
            <>
              ...
              <p>
                ...
                {selectedEvent.extendedProps.appointment?.driver?.user?.name ||
                  ""}{" "}
                {selectedEvent.extendedProps.appointment?.driver?.user
                  ?.surname || ""}
              </p>
              {selectedEvent.extendedProps.appointment?.driver?.user?.avatar
                ?.path ? (
                <img
                  src={
                    selectedEvent.extendedProps.appointment?.driver?.user
